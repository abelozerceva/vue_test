...
var TableCompvue_type_template_id_180c3249_scoped_true_lang_pug_render = function() {
  ...
  return _c("div", { staticClass: "wrapTable" }, [
    ...
    _c("div", { staticClass: "wrapTable__body" }, [
...
var TableCompvue_type_style_index_0_id_180c3249_scoped_true_lang_scss_ = __webpack_require__(58);
...
/* harmony default export */ var GlassStatevue_type_script_lang_js_ = ({
  ...
  methods: {
    async snapshot() {
      // делаем снепшот
      let glass = await this.$sdk.get(this.symbol); // переворачиваем, чтобы сверху таблицы были новые данные

      glass.bids = glass.bids.reverse();
      glass.asks = glass.asks.reverse(); // отрисовываем снепшот

      this.glass = glass;
    },

    wsSubscribe() {
      this.ws = this.$sdk.subscribe(this.symbol); // запускаем callBack при получении сообщений

      this.ws.onmessage = async event => {
        // парсим данные в Json
        let data = JSON.parse(event.data); // создаём массивы новых данных

        let [asksAdd, bidsAdd] = [// отфильтровываем 0-ые сделки
        data.a.filter(item => item[1] != 0), data.b.filter(item => item[1] != 0)]; // переворачиваем, чтобы сверху таблицы были новые данные

        asksAdd.reverse();
        bidsAdd.reverse(); // удаляем из буффера столько данных сколько пришло

        this.glass.asks.splice(this.glass.asks.length - asksAdd.length, asksAdd.length);
        this.glass.bids.splice(this.glass.bids.length - bidsAdd.length, bidsAdd.length); // дописываем полученные данные в буфер asks
...
var GlassStatevue_type_style_index_0_id_1bf15a4e_scoped_true_lang_scss_ = __webpack_require__(61);
